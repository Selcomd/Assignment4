...
public class LossCurtainController : MonoBehaviour
{
    ...
    private IEnumerator CloseCurtain()
    {
        ...
        while (Vector2.Distance(topCurtain.anchoredPosition, closedPosition) > 1f ||
               Vector2.Distance(bottomCurtain.anchoredPosition, closedPosition) > 1f)
        {
            topCurtain.anchoredPosition = Vector2.MoveTowards(
                topCurtain.anchoredPosition,
                closedPosition,
                closeSpeed * Time.deltaTime
            );

            bottomCurtain.anchoredPosition = Vector2.MoveTowards(
